...
function genmesh()
    ...
    mesh = Mesh()
    insert!(mesh, Q4block(A, A, N, N), "velocity+pressure")
    ir = baseincrel(mesh)
    eidir = identty(ir)
    insert!(mesh, eidir)
    return mesh
...
function run()
    ...
    vtkwrite("stokes_driven_q4-p", baseincrel(mesh), [(name = "p",), ])
    vtkwrite("stokes_driven_q4-v", baseincrel(mesh), [(name = "ux",), (name = "uy",)])
    true
end

end

stokes_driven_q4.run()
